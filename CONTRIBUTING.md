...
## Updating the demo recording

The README's terminal demo (`docs/public/demo.gif`) and its higher quality companion (`docs/public/intl-ai-demo.mp4`) are generated by [VHS](https://github.com/charmbracelet/vhs) from `docs/public/intl-ai-demo.tape`. VHS and [gum](https://github.com/charmbracelet/gum) are pinned in `mise.toml`.

### Layout

```
demo/
  setup-scratch.sh          # sourced by the tape: builds a throwaway scratch dir + intl-ai() shell function
  intl-ai.demo.config.json  # config used inside that scratch dir
  title-card.sh             # opening gum banner
  translating-hud.sh        # faked "Translating with AI..." beat (stands in for real LLM latency)
  closing-card.sh           # closing gum banner
docs/public/
  intl-ai-demo.tape         # the VHS script
  demo.gif                  # committed, embedded in README
  intl-ai-demo.mp4          # committed, higher quality cut for social
```

Every beat in the tape is a real, live command against a real build of the CLI, run inside a `mktemp -d` scratch directory (never against `examples/vite`, so nothing tracked gets dirtied). The only faked beat is the "Translating with AI..." HUD, which stands in for real LLM round-trip latency; the `fill` step right after it is a genuine API call.

Companion scripts are kept separate from the tape because VHS's `Type "..."` lexer cannot handle backslash-escaped quotes, and any `gum style` call with nested quotes needs its own file.

### Rendering

```bash
pnpm --filter @intl-ai/cli build   # demo scripts shell out to packages/cli/dist/index.mjs
chmod +x demo/*.sh
pnpm demo:render                   # runs `vhs docs/public/intl-ai-demo.tape`
```

The tape calls a real model through `demo/intl-ai.demo.config.json`. Point it at whatever OpenAI-compatible endpoint you have available (a local LM Studio/Ollama server, or a real provider key via an env var reference like `"apiKey": "${DEMO_API_KEY}"`). Nine short strings get translated per render, so cost is negligible even against a paid provider.

After rendering:

1. Check both output files are comfortably under 5MB (`ls -lh docs/public/demo.gif docs/public/intl-ai-demo.mp4`); trim `Width`/`Height`/`Framerate` in the tape before cutting narrative beats if not.
2. Pull a few frames and look at them before committing, to confirm no local paths or usernames leak onto screen: `ffmpeg -ss 4 -i docs/public/intl-ai-demo.mp4 -frames:v 1 /tmp/frame.png`.
